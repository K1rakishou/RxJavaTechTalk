...
class `11_schedulers` {
    ...
    /**
     *
     * Schedulers.io - Используется неограниченный пул потоков. Если в пуле есть свободный поток, вернёт его.
     * Если в пуле нет свободного потока вернёт новый поток и добавит его в пул. Может быть причиной ООМ.
     * Подходит для I/O задач, которые не юзают активно CPU (т.е. задач, которые в основном висят в
     * режиме ожидания - запрос к БД, к серверу, к диску и тд).
     *
     ...
     *
     * Schedulers.newThread - создаёт новый поток при каждом (не совсем) обращении к шедулеру. Лучше не использовать.
     * Если очень нужно, то следует заюзать Schedulers.from(Executor())
     *
